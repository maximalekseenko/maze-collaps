...
VisualComponent::VisualComponent(int __x, int __y, int __w, int __h, VisualComponent::Layer __layer, const char *__content, Color __colorB)
{
    ...
    this->layer = __layer;
    {
        ...
        if (this->colorB != Color::NONE)
            wbkgd(this->win, COLOR_PAIR(GetColorPairId(0, this->colorB)));
...
int VisualComponent::GetW()
{
    std::lock_guard curses_locker(Curses::curses_lock);
    return getmaxx(this->win);
}
int VisualComponent::GetH()
{
    std::lock_guard curses_locker(Curses::curses_lock);
    return getmaxy(this->win);
}
int VisualComponent::GetMaxX()
{
    std::lock_guard curses_locker(Curses::curses_lock);
    return GetX() + GetW();
}
int VisualComponent::GetMaxY()
{
    std::lock_guard curses_locker(Curses::curses_lock);
    return GetY() + GetH();
}
...
{
    return __me->GetX() <= __x && __me->GetMaxX() > __x && __me->GetY() <= __y && __me->GetMaxY() > __y;
}
...
bool VisualComponent::Hover(int __x, int __y)
{
    ...
    if (this->hovered == false && __on == false)
        return false;
...
bool VisualComponent::Click(int __x, int __y)
{
    ...
    if (!IsPointWithinMe(__x, __y, this))
        return false;
