...
class OverlappingModel : Model {
    ...
    public: OverlappingModel(std::string name, int N, int width, int height, bool periodicInput, bool periodic, int symmetry, bool ground, Heuristic heuristic)
        : Model(width, height, N, periodic, heuristic)
    {
        ...
        patterns = new();
        std::dictionary<long, int> patternIndices;
        ...
        for (int y = 0; y < ymax; y++) for (int x = 0; x < xmax; x++)
            {
                ...
                for (int k = 0; k < symmetry; k++)
                {
                    ...
                    {
                        patternIndices.Add(h, weightList.size());
                        weightList.Add(1.0);
                        patterns.Add(p);
                    }
                }
