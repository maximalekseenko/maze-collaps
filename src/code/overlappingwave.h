...
class OverlappingWave : public Model {
    ...
    public: // +++CONSTRUCTORS AND DESTRUCTORS+++
        ...
        OverlappingWave(std::string name, int N, int width, int height, bool periodicInput, bool periodic, int symmetry, bool ground, Heuristic heuristic)
    ...
    int* Result()
        {
            ...
            for (int y = 0; y < MY; y++)
            {
                ...
                for (int x = 0; x < MX; x++)
                {
                    int dx = x < MX - N + 1 ? 0 : N - 1;
                    if (observed[x - dx + (y - dy) * MX] == -1) std::cout << '*';
                    else finalArray[x + y * MX] = colors[patterns[observed[x - dx + (y - dy) * MX]][dx + dy * N]];
                }
            }
            return finalArray;
        }
